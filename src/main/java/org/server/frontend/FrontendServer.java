 ...
 */
public class FrontendServer extends
		AbstractRMIServerClient<FrontendServerConfig> {
	...
	// 套接字事件处理器
	final FrontendSocketEventHandler _frontendSocketEventHandler = new FrontendSocketEventHandler(
			this);
	// 前台服务节点编号
	int _nodeId = 0;
	// 连接错误提示
	...
	public FrontendServer() {
		...
		timer = Schedule.scheduleWithFixedDelay(this::tryTestConnection,
				LocalDateTime.now(), Duration.ofMillis(checkTimeMillis));
	}
	...
	boolean createBackendServerConnection() {
		...
		synchronized (_backendLock) {
			try {
				Registry masterRegistry = LocateRegistry.getRegistry(address,
						port, new SslRMIClientSocketFactory());

				_backendServer = (BackendRMIServerInterface) masterRegistry
						.lookup("BackendServer");
				_backendServer.ping();
				...
				if (!_singleErrorTip) {
					logger.error(" FrontendServer  自动连接前台服务失败 :: ({}:{}:{})",
	...
	@Override
	protected RMIServiceInterfaceImpl<?> createMasterInterfaceImpl()
