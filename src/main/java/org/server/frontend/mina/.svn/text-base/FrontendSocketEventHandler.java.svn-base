...
public class FrontendSocketEventHandler {
    ...
    private static final Logger logger = LoggerFactory.getLogger(FrontendSocketEventHandler.class);
     ...
     *
     * @param id 会话编号
     * @param sessionMessage 消息对象
     */
    public void fireSessionReceived(long id, SessionMessage sessionMessage) {
        callRemote(s -> s.onSessionReceived(id, sessionMessage, getServer().getNodeId()));
    }
    ...
    void fireSessionIdle(long id, IdleStatus status) {
        callRemote(s -> s.onSessionIdle(id, SessionIdleType.parse(status), getServer().getNodeId()));
    }
    ...
    public String getAddress(long sessionId) {
        IoSession session = getServer().getAcceptor().getManagedSessions().get(sessionId);
        try {
            if (session != null) {
