...
/**
 * runnable execute factory
 * 
 * @author Hxms
 *         
 */
public class RunnableExecuteFactory {
    
    String          executeFactoryName;
    int             usingThreadCount;
    ExecutorService executeService;
                    
    public RunnableExecuteFactory(String factoryName) {
    ...
    public void doWorker(Runnable runnable) {
        if (executeService != null) executeService.execute(runnable);
    }
}
