...
public class TransportCallback<M extends GeneratedMessage, U> {
    ...
    public TransportCallback(
            short command,
            MessageCallback<M> callback,
            CreateSharedModule<M> createSharedModule) {
    ...
    public TransportCallback(
            short _command,
            CreateSharedModule<M> _createSharedModule,
            VerifyMessageCallback<M, U> _verifyMessageCallback) {
    ...
    public void received(GameSession session, SessionMessage data) {
        ...
        boolean messageModelVerify
                = (_createSharedModule == null
                || (_createSharedModule != null && obj != null));
        ...
        boolean verify = _verifyMessageCallback != null
                && Transport.getAuthentication() != null;
        if (verify && messageModelVerify) {
            _ObjectuserModel = Transport.getAuthentication().authentication(session);
            if (_ObjectuserModel != null) {
                try {
                    _verifyMessageCallback.callback(session, data, (U) _ObjectuserModel, obj);
                } catch (Exception e) {
                    logger.error("Transport MessageCallback Error:", e);
                }
            }
        ...
        if (_verifyMessageCallback != null
                ...
                && messageModelVerify
                && _ObjectuserModel == null // 无验证对应用户模块
                ) {
            Transport.getAuthenticationFail().authenticationFail(session);
        }

        // 认证失败
//        if (_verifyMessageCallback != null && !verify) {
//            logger.warn(String.format(
//                    "接收消息认证失败：游戏会话 [%s],消息头值 [0x%s]",
//                    session.getBackendSession().toString(),
//                    Integer.toHexString(_command).toUpperCase()));
//        }
    }
}
