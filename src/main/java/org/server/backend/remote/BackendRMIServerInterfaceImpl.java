...
public class BackendRMIServerInterfaceImpl extends RMIServiceInterfaceImpl<BackendServer> implements BackendRMIServerInterface {
    ...
    private static final Logger logger = LoggerFactory.getLogger(BackendRMIServerInterfaceImpl.class);
    /**
     * 服务集合
     */
     ...
     * 调用远程对象方法
     *
     * @param frontendId 前台服务编号
     * @param action 执行函数
     */
    private void callRemote(int frontendId, RemoteObjectAction<FrontendBackendInterface> action) {
        try {
     ...
     * @param action 执行函数
     */
    private <R> R callRemoteFunc(int frontendId, RemoteObjectFunc<FrontendBackendInterface, R> action) {
        try {
            FrontendBackendInterface frontendBackendInterface = _frontendServer.get(frontendId);
            if (frontendBackendInterface != null) {
