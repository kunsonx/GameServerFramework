...
public class BackendRMIServerInterfaceImpl extends
		RMIServiceInterfaceImpl<BackendServer> implements
		BackendRMIServerInterface {
	...
	@Override
	public void registerFrontendCore(FrontendBackendInterface service)
			throws RemoteException {
		try {
			// 测试通讯
			service.ping();
	...
	public void onSessionClosed(long id, int nodeId) throws RemoteException {
		GameSessionProvider.getInstance().dispose(
				new BackendSession(nodeId, id));
	}

	@Override
	public void onSessionIdle(long id, SessionIdleType status, int nodeId)
			throws RemoteException {
	}
	...
	public void write(BackendSession session, SessionMessage message) {
		callRemote(session.getFrontendServerId(),
				f -> f.write(session.getSessionId(), message));
	}
	...
	private void callRemote(int frontendId,
		...
		try {
			FrontendBackendInterface frontendBackendInterface = _frontendServer
					.get(frontendId);
			...
			logger.error("[BackendServer] 连接前台服务对象错误："
