...
public abstract class AbstractRMIServerClient<C extends AbstractServerConfig> extends AbstractStandardServer<C> {
     ...
     * @throws NotBoundException 未绑定异常
     */
    ...
    private boolean createRMIServer() {
        try {
            ...
            _register = LocateRegistry.createRegistry(
                    getServerConfig().getRMIAddress().isValid()
                            ? getServerConfig().getRMIAddress().getPort()
                            : getServerConfig().getMasterPort(),
                    new SslRMIClientSocketFactory(),
